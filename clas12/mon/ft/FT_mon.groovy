...
*/
  ...
  def finish() {
    ...
    def hsnorm0 = [24,25].collect{new H2F("full/h2enorm_b${it}", "normalized number of ${it}th trigger bits [full range];normalized number of trigger bits", 200,0,maxnorm,200,0,60)}
    def hsnorm1 = [24,25].collect{new H2F("zoom/h2enorm_b${it}", "normalized number of ${it}th trigger bits [fixed range];normalized number of trigger bits", 200,0,15,200,0,60)}
    ...
    [hsntrgs,hsnorm0,hsnorm1].each{it.each{hists[it.getName().replace("h2","h")] = it.projectionX(it.getName().replace("h2","h"))}}
